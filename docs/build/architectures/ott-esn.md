...
The default choice for chaotic attractor reconstruction: a classic ESN with
one extra transform that squares half the reservoir states before the
readout.
...
sees odd-order terms — an autonomous forecast can settle onto the mirror
image of the attractor. Squaring half the units hands the readout
even-order features and breaks that symmetry, while the concatenation keeps
the raw input available as in [classic_esn](classic-esn.md).
...
Approach*, Phys. Rev. Lett. **120**, 024102 (2018). The architecture is
popularly associated with Edward Ott's group — hence the factory name —
though the paper's first author is Pathak.

## See also

- [power_augmented](power-augmented.md) — the same idea with a sweepable exponent.
