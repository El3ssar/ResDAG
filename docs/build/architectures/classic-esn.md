...
The textbook baseline, with the one wrinkle worth knowing up front: the
readout sees the raw input *next to* the reservoir states, not the states
alone.
...
the concatenation, so the readout solves ridge regression over
...
!!! note "Choosing a baseline"
    Reach for `classic_esn` when you want the standard against which the
    augmented variants are judged. For chaotic attractor reconstruction,
    start from [ott_esn](ott-esn.md) instead.
